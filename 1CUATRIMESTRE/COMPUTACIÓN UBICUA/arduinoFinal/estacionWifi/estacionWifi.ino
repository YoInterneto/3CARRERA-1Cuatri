...
void loop() {
  ...
  readVeleta(&viento[0]);
  readMQ135(&mq[0]);
  String httpRequestData = String("?id=") + id_estacion + "&humedad=" + hV  +"&temperatura=" + tV  +"&sensacion_termica=" + hicV +"&presion_atmosferica=" +bmp[0] 
...
void readVeleta(float *array){
   ...
   double y = map(YValue, 1121, 4095, -100, 100); // los valores min y max se deben ajustar a cada sensor
   double x = map(XValue, 1040, 4095, -100, 100);
   array[0] = x;
   array[1] = y;
